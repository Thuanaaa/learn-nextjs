...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        alert(`Đăng nhập thành công! Xin chào ${response.data.user.fullName || response.data.user.username}`);
      ...
      if (apiError.code === 'INVALID_CREDENTIALS' || apiError.status === 401) {
        ...
        setErrors({ general: 'Lỗi kết nối. Vui lòng kiểm tra internet của bạn.' });
      } else if (apiError.code === 'TIMEOUT') {
        setErrors({ general: 'Yêu cầu quá lâu. Vui lòng thử lại.' });
      } else {
        setErrors({ general: apiError.message || 'Đã có lỗi xảy ra. Vui lòng thử lại.' });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-20 left-10 w-20 h-20 bg-purple-500 rounded-full opacity-20 animate-float"></div>
      <div className="absolute bottom-20 right-10 w-32 h-32 bg-pink-500 rounded-full opacity-20 animate-float" style={{ animationDelay: '1s' }}></div>
      ...
      <div className="w-full max-w-md animate-fadeIn">
        ...
        <div className="card">
          ...
          {errors.general && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/50 rounded-xl animate-slideIn">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {errors.username && (
                <p className="mt-1 text-sm text-red-500 animate-slideIn">{errors.username}</p>
              )}
            </div>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-transparent text-gray-400">Hoặc đăng nhập với</span>
            </div>
          </div>
